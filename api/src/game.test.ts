...
describe('decideRound (RPSLS)', () => {
  ...
  it('lizard beats spock and paper', () => {
    expect(decideRound('lizard', 'spock')).toBe('a');
    expect(decideRound('lizard', 'paper')).toBe('a');
  });

  it('spock beats scissors and rock', () => {
    expect(decideRound('spock', 'scissors')).toBe('a');
    expect(decideRound('spock', 'rock')).toBe('a');
  });

  it('is symmetric (loser perspective)', () => {
    expect(decideRound('scissors', 'rock')).toBe('b');
    expect(decideRound('spock', 'paper')).toBe('b');
  });

  it('identical moves draw', () => {
    expect(decideRound('spock', 'spock')).toBe('draw');
...
describe('isMove', () => {
  it('accepts the five valid moves', () => {
    for (const m of ['rock', 'paper', 'scissors', 'lizard', 'spock']) {
...
describe('delay marks (cooldown system)', () => {
  it('starts with lizard at 1 and spock at 2', () => {
    expect(INITIAL_DELAYS).toEqual({ rock: 0, paper: 0, scissors: 0, lizard: 1, spock: 2 });
  ...
  it('after a choice: all decrement by 1, chosen gains 2', () => {
    ...
    expect(d).toEqual({ rock: 2, paper: 0, scissors: 0, lizard: 0, spock: 1 });
  ...
  it('replays a sequence to the right state', () => {
    // r1 rock -> {rock2, spock1, lizard0}; r2 lizard -> decrement then +2 lizard
    const d = computeDelays(['rock', 'lizard']);
    expect(d).toEqual({ rock: 1, paper: 0, scissors: 0, lizard: 2, spock: 0 });
  });
