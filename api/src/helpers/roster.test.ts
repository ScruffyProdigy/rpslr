...
describe('helper roster', () => {
  ...
  it('prices each ability at the cadence its effect is worth', () => {
    ...
    expect(marks).toEqual({
      ...
      flywheel: '0/4',
      feint: '0/3',
      // Quarantine's secret twin (JQ-236). Same +2, one round faster: unannounced,
      // it is collected on the ~1/3 who walk into it, where the public one is dodged
      // and priced on the option it removes instead. Visibility sets the price and
      // the price sets the cadence, which is the whole of the pair.
      tripwire: '1/2',
    });
  });
