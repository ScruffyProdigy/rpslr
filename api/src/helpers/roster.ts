...
const MAJORS = [
  ...
  //
  // Recharge 2 rather than 3, per the correction above the `Load` type: a recharge is
  // rounds *unavailable*, so the gap between firings is one more than the number.
  // This fires rounds 2, 5, 8 — a third of them — which is the cadence ~8.5pp is
  // priced on.
  //
  // `opening: 1` is the doc's rule that a pair meant to be drafted together should
  // not both open at 0, and this pair is drafted together by definition, since being
  // compared is why both exist. It bites because these two *compound*: Quarantine is
  // announced, so the opponent steps off the move it names and picks from two rather
  // than three, which lifts this card's secret guess from a 1/3 hit to a 1/2.
  // Ungated they coincide on round 1, putting the pair's strongest moment where
  // nobody has anything to read. The mark moves it to round 5, with four rounds of
  // history on the table, and costs no firings at all.
  //
